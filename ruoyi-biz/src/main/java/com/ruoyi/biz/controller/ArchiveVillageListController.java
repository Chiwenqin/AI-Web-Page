...
@GetMapping("/list")
    public TableDataInfo list(HttpServletRequest request, ArchiveVillageList archiveVillageList) {
        QueryWrapper<ArchiveVillageList> queryWrapper = new QueryWrapper<>();

        // 手动构建查询条件，避免驼峰命名转换问题
        // 获取前端传递的参数（兼容 Like 后缀的参数名）
        String villageCode = request.getParameter("villageCodeLike");
        if (villageCode != null && !villageCode.isEmpty()) {
            queryWrapper.like("village_code", villageCode);
        }
        String villageName = request.getParameter("villageNameLike");
        if (villageName != null && !villageName.isEmpty()) {
            queryWrapper.like("village_name", villageName);
        }
        String villageCount = request.getParameter("villageCountLike");
        if (villageCount != null && !villageCount.isEmpty()) {
            queryWrapper.like("village_count", villageCount);
        }
        String villageEntrance = request.getParameter("villageEntranceLike");
        if (villageEntrance != null && !villageEntrance.isEmpty()) {
            queryWrapper.like("village_entrance", villageEntrance);
        }
        String villageHouse = request.getParameter("villageHouseLike");
        if (villageHouse != null && !villageHouse.isEmpty()) {
            queryWrapper.like("village_house", villageHouse);
        }
        String floorArea = request.getParameter("floorAreaLike");
        if (floorArea != null && !floorArea.isEmpty()) {
            queryWrapper.like("floor_area", floorArea);
        }
        String billingArea = request.getParameter("billingAreaLike");
        if (billingArea != null && !billingArea.isEmpty()) {
            queryWrapper.like("billing_area", billingArea);
        }
        String villageCompany = request.getParameter("villageCompanyLike");
        if (villageCompany != null && !villageCompany.isEmpty()) {
            queryWrapper.like("village_company", villageCompany);
        }
        ...
        if (params != null && !params.isEmpty()) {
            try {
                cn.hutool.json.JSONObject json = cn.hutool.json.JSONUtil.parseObj(params);
                if (json.containsKey("beginTime") && json.getStr("beginTime") != null) {
                    queryWrapper.ge("create_datetime", json.getStr("beginTime"));
                }
                if (json.containsKey("endTime") && json.getStr("endTime") != null) {
                    queryWrapper.le("create_datetime", json.getStr("endTime"));
                }
            } catch (Exception e) {
                // 解析失败忽略
            }
    ...
    public AjaxResult add(@Validated @RequestBody ArchiveVillageList archiveVillageList) {
            // 自动填充创建人和创建时间
            archiveVillageList.setCreatePerson(getUsername());
            archiveVillageList.setCreateDatetime(new java.util.Date());
            // 同时填充更新人和更新时间
            archiveVillageList.setUpdatePerson(getUsername());
            archiveVillageList.setUpdateDatetime(new java.util.Date());
